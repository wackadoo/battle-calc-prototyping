...
def parse_classes(params)
  ...
  re = Hash.new
  params.each {|x| re[x["name"]] = Bc::UnitClass.new(x["name"]) unless (!x.has_key?("name") || x["name"].empty?) }
  re
end

def get_type_float_field_setters
  re = Hash.new
  dummy = Bc::UnitType.new
  Bc::UnitType.instance_methods.reject{ 
    |x| 
...
get '/' do
  ...
  fight_done = false
  @fight_after = Bc::Fight.new(@fight)
